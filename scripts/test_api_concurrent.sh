...
export BASE_URL
export PAYLOAD='{"c_code":"int foo(void){return 1;}","rust_code":"pub fn foo()->i32{1}"}'
...
count=$(printf "%s" "${responses}" | python -c 'import sys; data=sys.stdin.read(); print(data.count("cosine_similarity"))')
